...
SYSTEM_PROMPT = f"""Ты — редактор ежедневной подборки самых популярных постов с картинками про игру
...
Твоя задача:
1. Убери повторы — если один и тот же пост/картинка встречается несколько раз (репосты
   между сабреддитами или дублирующиеся находки), оставь только запись с наименьшим
   (то есть более высоким по популярности) rank.
2. Убери NSFW/откровенный контент, оскорбительные или разжигающие ненависть картинки, а
   ...
   находка или спекуляция — такое тоже нужно оставлять. Оценивай КАЖДЫЙ пост отдельно по
   его содержанию — то, что пост пришёл из сабреддита r/GTA6unmoderated, само по себе не
   повод его исключать.
3. Из оставшегося выбери не более {MAX_MEMES} самых популярных, отсортируй по возрастанию
   rank (самый популярный — первым). Если реальных постов за день меньше {MAX_MEMES} —
